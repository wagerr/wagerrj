 ...
 * {@link org.wagerrj.crypto.DeterministicHierarchy}, to derive all the keys in the keychain from a master seed.
 ...
 * written down on paper or, when represented using a BIP 39 {@link org.wagerrj.crypto.MnemonicCode},
 ...
 * {@link org.wagerrj.crypto.DeterministicKey#serializePubB58(org.wagerrj.core.NetworkParameters)}. The resulting "xpub..." string encodes
 * sufficient information about the account key to create a watching chain via
 * {@link org.wagerrj.crypto.DeterministicKey#deserializeB58(org.wagerrj.crypto.DeterministicKey, String, org.wagerrj.core.NetworkParameters)}
 * (with null as the first parameter) and then
 * {@link DeterministicKeyChain#DeterministicKeyChain(org.wagerrj.crypto.DeterministicKey,KeyChainType keyChaintype)}.</p>
 *
 * <p>This class builds on {@link org.wagerrj.crypto.DeterministicHierarchy} and
 * {@link org.wagerrj.crypto.DeterministicKey} by adding support for serialization to and from protobufs,
...
public class DeterministicKeyChain implements EncryptableKeyChain {
    ...
    public static final ChildNumber WAGERR_PATH = new ChildNumber(CoinDefinition.BIP_44_COIN_TYPE,true);
    public static final ImmutableList<ChildNumber> BIP44_ACCOUNT_ZERO_PATH =
            ImmutableList.of(BIP44_MASTER_KEY, WAGERR_PATH, ChildNumber.ZERO_HARDENED);
    ...
    public static class Builder<T extends Builder<T>> {
        ...
        public DeterministicKeyChain build() {
            ...
            if (random != null) {
                ...
                chain = new DeterministicKeyChain(watchingKey,KeyChainType.BIP44_WAGERR_ONLY);
            }
    ...
    protected DeterministicKeyChain(DeterministicSeed seed) {
        this(seed, null,KeyChainType.BIP44_WAGERR_ONLY);
    }
    ...
    public DeterministicKeyChain(DeterministicKey watchingKey,KeyChainType keyChainType) {
        ...
        if (keyChainType != KeyChainType.BIP44_WAGERR_ONLY)
    ...
    protected DeterministicKeyChain(DeterministicKey watchKey, boolean isFollowing) {
        this(watchKey,KeyChainType.BIP44_WAGERR_ONLY);
    ...
    public static DeterministicKeyChain watch(DeterministicKey accountKey) {
        return new DeterministicKeyChain(accountKey,KeyChainType.BIP44_WAGERR_ONLY);
    }
    ...
    private void initializeHierarchyUnencrypted(DeterministicKey baseKey) {
        if (baseKey.isPubKeyOnly() && keyChainType == KeyChainType.BIP44_WAGERR_ONLY){
    ...
    public List<DeterministicKey> getKeys(KeyPurpose purpose, int numberOfKeys) {
        ...
        try {
            ...
            for (int i = 0; i < numberOfKeys; i++) {
                ...
                // by any real problem reports from wagerrj users, but I've heard of cases via the grapevine of
     ...
     * The returned key can be serialized and then passed into {@link #watch(org.wagerrj.crypto.DeterministicKey)}
    ...
    public static List<DeterministicKeyChain> fromProtobuf(List<Protos.Key> keys, @Nullable KeyCrypter crypter, KeyChainFactory factory) throws UnreadableWalletException {
        ...
        KeyChainType keyChainType = KeyChainType.BIP44_WAGERR_ONLY;
        ...
        for (Protos.Key key : keys) {
            ...
            if (!path.isEmpty() && path.get(0).equals(BIP44_MASTER_KEY)){
                // is BIP44
                keyChainType = KeyChainType.BIP44_WAGERR_ONLY;
                break;
            }
        }
    ...
    public DeterministicKeyChain toEncrypted(KeyCrypter keyCrypter, KeyParameter aesKey) {
        return toEncrypted(keyCrypter,aesKey,KeyChainType.BIP44_WAGERR_ONLY);
    }
     ...
     * @see org.wagerrj.wallet.MarriedKeyChain
     */
