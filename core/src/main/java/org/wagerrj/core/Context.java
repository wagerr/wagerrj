...
import org.wagerrj.core.listeners.BlockChainListener;
import org.wagerrj.store.FlatDB;
import org.wagerrj.store.HashStore;
import org.wagerrj.store.MasternodeDB;
 ...
 * wagerrj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating wagerrj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use wagerrj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a wagerrj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using wagerrj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing wagerrj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.wagerrj.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.wagerrj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.wagerrj.core.NetworkParameters} specified when this context was (auto) created. The
    ...
    public void initwagerr(boolean liteMode, boolean allowInstantX) {
    ...
    public void setPeerGroupAndBlockChain(PeerGroup peerGroup, @Nullable AbstractBlockChain chain)
    {
        ...
        if (sporkManager!=null) {
            ...
            log.error("##### wagerr init not called!, this is going to be an issue in the future");
        }
        if (instantSend!=null) {
    ...
    BlockChainListener updateHeadListener = new BlockChainListener () {
        ...
        {
            //todo furszy: commented wagerr MN stuff
